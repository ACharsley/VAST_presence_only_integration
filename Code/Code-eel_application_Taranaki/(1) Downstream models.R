###################################################
##        Model 1a for the Taranaki region       ##
##                                               ##
##                Downstream model               ##
##                                               ##
##               Anthony Charsley                ##
...
# This code builds 'downstream' models with presence/absence 
...
##Network
network <- taranaki_data$network_ds
...
##Habitat data
X_gctp <- taranaki_data$X_gctp_ds

##NZFFD / presence-only / pseudo-absence data
obs <- taranaki_data$obs_ds
...
if(data_indicator == "Taranaki data") save.image(file.path(data_taranaki_dir, paste0("general_inputs_taranaki_ds.Rdata")))
if(data_indicator == "Sample1a") save.image(file.path(data_taranaki_dir, paste0("general_inputs_sample1a_ds.Rdata")))
if(data_indicator == "Sample1b") save.image(file.path(data_taranaki_dir, paste0("general_inputs_sample1b_ds.Rdata")))
if(data_indicator == "Sample1c") save.image(file.path(data_taranaki_dir, paste0("general_inputs_sample1c_ds.Rdata")))
if(data_indicator == "Sample1d") save.image(file.path(data_taranaki_dir, paste0("general_inputs_sample1d_ds.Rdata")))
...
######################################################################
# Habitat unsuitability model 1 - no covariates w/ catchability term #
######################################################################
# Build HUM_1_ds as follows:
#   - With no habitat covariates
...
if(data_indicator == "Taranaki data") load(file.path(data_taranaki_dir, paste0("general_inputs_taranaki_ds.Rdata")))
...
if(data_indicator == "Taranaki data") path <- file.path(model_path, "HUM_1_ds")
...
#######################################################################
# Habitat unsuitability model 2 - w/ covariates and catchability term #
#######################################################################
# Build HUM_2_ds as follows:
...
if(data_indicator == "Taranaki data") path <- file.path(model_path, "HUM_2_ds")
...
## This model is unnecessary (doesn't make sense)

# #########################
# # Model - no covariates #
# #########################
# # Build Model1a_1_ds as follows:
# #   - Using Sample 1a downstream data
# #   - With no habitat covariates
# #   - Without data type as a spatially varying catchability covariate
# #   - Without 'sampler' as a cacthability covariate
# 
# 
# library(tidyverse)
# library(VAST)
...
# #############
# 
# load(file.path(data_taranaki_dir, paste0("general_inputs_sample1a_ds.Rdata")))
# 
# 
# ####################
# #  Call functions  #
...
# 
# # 3. Set up catchability input
# # Q1_formula <- ~ Data_type_input
# # Q1config_k <- 3
# # Q2_formula <- ~ 1 #The default is "~0". Is this correctly specified
# # Q2config_k <- NULL
# # catchability_data <- Data_inp[,c("Lat", "Lon", "Data_type_input")]
# 
# Q1_formula <- ~ 0
# Q1config_k <- NULL
# Q2_formula <- ~ 0
# Q2config_k <- NULL
# catchability_data <- data.frame("Data_type_input"=Data_inp$Data_type_input)
# 
# 
# # 4. Set model settings
# Version = "VAST_v14_0_1" #Version = "VAST_v9_0_0"
# 
# FieldConfig <- c("Omega1" = 1, "Epsilon1" = 1, "Omega2" = 0, "Epsilon2" = 0) #1 category
# RhoConfig <- c("Beta1" = 2, "Beta2" = 3, "Epsilon1" = 1, "Epsilon2" = 0) #Remove RhoConfig[3] if necessary
# 
...
##################################################################
# Model 1 - no covariates w/ spatially varying catchability term #
##################################################################
# Build Model1a_2_ds as follows:
#   - Using Sample 1a downstream data
#   - With no habitat covariates
#   - With data type as a spatially varying catchability covariate
#   - Without 'sampler' as a cacthability covariate
...
data_indicator == "Sample1a"

if(data_indicator == "Sample1a") load(file.path(data_taranaki_dir, paste0("general_inputs_sample1a_ds.Rdata")))
if(data_indicator == "Sample1b") load(file.path(data_taranaki_dir, paste0("general_inputs_sample1b_ds.Rdata")))
if(data_indicator == "Sample1c") load(file.path(data_taranaki_dir, paste0("general_inputs_sample1c_ds.Rdata")))
if(data_indicator == "Sample1d") load(file.path(data_taranaki_dir, paste0("general_inputs_sample1d_ds.Rdata")))
...
if(data_indicator == "Sample1a") path <- file.path(model_path, "Model1a_1_ds")
if(data_indicator == "Sample1b") path <- file.path(model_path, "Model1b_1_ds")
if(data_indicator == "Sample1c") path <- file.path(model_path, "Model1c_1_ds")
if(data_indicator == "Sample1d") path <- file.path(model_path, "Model1d_1_ds")
...
##NOTE: First need to investigate principal components model to
##      determine what covariates are needed.

##################################################################
# Model 3 - Covariates w/ spatially varying catchability term #
##################################################################
# Build Model1a_2_ds as follows:
#   - Using Sample 1a downstream data
#   - With no habitat covariates
...
# Create model path
path <- file.path(model_path, "Model1a_2_ds")
dir.create(path, showWarnings = FALSE)
path_figs <- file.path(path, "Figures")
dir.create(path_figs, showWarnings=FALSE)
