...
X1_formula_inp = X2_formula_inp = ~std_log_loc_elev + std_SegRipShade_sqrd + std_log_MeanFlowCumecs + std_FWENZ_segSubstrate + std_local_twarm + factor(Barrier_present)
...
if(scenario == "Taranaki data"){
  Data_inp$Data_type_inp <- as.factor(ifelse(Data_inp$Data_type == "Structured_EF", 0, 1)) #Structured_EF = 0, Structured_NetTrap = 1
  
  Q1_formula <- ~ Data_type_inp
  Q1config_k <- 3
  Q2_formula <- ~ 0
  Q2config_k <- NULL
  catchability_data <- Data_inp[,c("Lat", "Lon", "Data_type_inp")]
}else{
  Data_inp$Data_type_inp <- as.factor(ifelse(Data_inp$Data_type == "Structured_EF", 0, 
                                   ifelse(Data_inp$Data_type == "Structured_NetTrap", 1, 2)) )
