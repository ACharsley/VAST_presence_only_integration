...
## Changed on re-run: ##
TmbData_new$FieldConfig["Epsilon","Component_1"] <- -1 
TmbData_new$RhoConfig <- c("Beta1" = 2, "Beta2" = 3, "Epsilon1" = 0, "Epsilon2" = 0) 


TmbData = make_data(b_i = as_units(Fit$data_frame$b_i, unitless), 
                    a_i = as_units(rep(1, nrow(Fit$data_frame)), unitless), 
                    t_i = Fit$data_frame$t_i,
                    
                    Version = Fit$settings$Version,
                    FieldConfig = c("Omega1" = 1, "Epsilon1" = 0, "Omega2" = 0, "Epsilon2" = 0), #Switches off Epsilon in component 1
                    OverdispersionConfig = Fit$settings$OverdispersionConfig,
                    RhoConfig = c("Beta1" = 2, "Beta2" = 3, "Epsilon1" = 0, "Epsilon2" = 0), #no structure on spatio-temporal term as now turned off
                    ObsModel_ez = Fit$settings$ObsModel, 
                    Options = Fit$settings$Options, 
                    
                    X_gctp = Fit$data_list$X1_gctp,
                    X_ip = Fit$data_list$X1_ip,
                    X1_formula = Fit$X1_formula,
                    X1config_cp = Fit$X1config_cp,
                    X2_formula = Fit$X2_formula,
                    X2config_cp = Fit$X2config_cp,
                    
                    catchability_data = Fit$catchability_data,
                    Q1_formula = Fit$Q1_formula,
                    Q2_formula = Fit$Q2_formula,
                    Q1config_k = Fit$Q1config_k,
                    Q2config_k = Fit$Q2config_k,
